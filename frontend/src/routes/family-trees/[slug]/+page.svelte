<script lang="ts">
  ...
  const syncNodesAndEdges = () => {
    ...
    const calculatePositions = (familyMember: any, level: number) => {
      ...
      if (levelMap[familyMember.id] === undefined || levelMap[familyMember.id] < level) {
        ...
        const children = family.family_members.filter((fm: any) =>
          fm.parent_ids && fm.parent_ids.includes(familyMember.id)
        );
  ...
  const saveFamilyTreeState = () => {
    ...
    client.restClient?.familyMembers.postV1FamilyMembers(
      familyMembers
    ).then(_ => {}).catch(error => {
      toastStore.update((prevValue) => ({
        ...prevValue,
    ...
    client.restClient?.familyMembers.putV1FamilyMembers(
      relationships
    ).then(res => {
    }).catch(error => {
      toastStore.update((prevValue) => ({
...
{:else if family && nodes && edges}
  ...
  <div class="mt-4 mb-4">
    ...
    {#if updateAuthContext.userActionPermitted(family.user_id, user.id)}
      <Button on:click={() => deleteFamilyTree(family.id)}>Delete Comment</Button>
    {/if}
    <Button>Reset</Button>
  </div>
